...
from apis.reddit.reddit_posts_saver import save_reddit_posts_to_csv
...
def sentiment_agent(state: FundState):
    ...
    try:
        save_reddit_posts_to_csv(
            posts=reddit_posts,
            ticker=ticker,
            trading_date=trading_date,
            data_source_type="ticker-relevant",
            exp_name=exp_name
        )
    except Exception as e:
        logger.warning(f"Failed to save Reddit posts to CSV for {ticker}: {e}")
